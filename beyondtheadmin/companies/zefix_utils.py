...
def convert_search_item_to_company_data(result_item):
    ...
    if 'address' in result_item:
        ...
        address = (result_item['address'].get('street') + ' ' + result_item['address'].get('houseNumber')).strip()
...
def search_zefix(company_name):
    ...
    response = requests.post(ZEFIX_SEARCH_URL, json=data,
                             auth=requests.auth.HTTPBasicAuth(username, password))
    if response.status_code == 200:
        return [convert_search_item_to_company_data(result_item) for result_item in response.json()]
    return []
