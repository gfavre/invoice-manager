...
class CompanyInvoiceListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        company_obj = get_object_or_404(
            Company, pk=self.kwargs.get("company_pk"), users=self.request.user
        )
    ...
    def get_search_panes(self):
        ...
        invoice_status = (
            invoices.values("status").annotate(total=Count("status")).order_by("total")
        )
        current_date = now()
        waiting_invoices_qs = invoices.filter(
            due_date__gte=current_date, status=Invoice.STATUS.sent
        )
        overdue_invoices_qs = invoices.filter(
            due_date__lt=current_date, status=Invoice.STATUS.sent
        )
    ...
    def get_object(self):
        try:
            ...
            transaction.on_commit(
                lambda: generate_pdf.delay(
                    invoice_id=str(invoice.id), version=version, content=content
                )
            )
            return invoice_pdf
        except ValueError:
